# Opis:
Budowanie z klocków Lego.

## Czego siê nauczy³em?
Zsynchronizowa³em grê przy u¿yciu Socket.IO.

### Wykorzystane technologie:
HTML, CSS, JavaScript, jQuery, Node.js, Three.js, Socket.IO

#### Uruchomienie:
Aby uruchomiæ projekt musimy mieæ zainstalowany Node.js.
Po pobraniu otworzyæ konsolê w folderze projektu (przytrzymaæ SHIFT i klikn¹æ prawym na folder, nastêpnie z menu podrêcznego wybraæ "Otwórz tutaj okno programu Powershell" lub podobne)).
W okienku konsoli wpisaæ "node server.js" lub "node .\server.js".
Jeœli polecenie nie dzia³a byæ mo¿e wywo³ywane jest w folderze, w którym nie ma pliku "server.js".
Jeœli wszystko dobrze zrobiliœmy wyœwietli siê napis "Listening on 3000".
Przechodzimy do przegl¹darki i w pasku adresu wpisujemy "localhost:3000".
Powinien otworzyæ siê projekt.

##### Dzia³anie projektu:
Budujemy struktury z klocków.
W grze znajduje siê opis wszystkich skrótów klawiszowych.
Po otwarciu projektu w kolejnej karcie przegl¹d¹darki to co budujemy pojawi siê u ka¿dego innego gracza (mo¿e graæ wielu graczy naraz).

![1](./images/lego1.png) 
![2](./images/lego2.png) 

